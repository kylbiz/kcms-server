...
Accounts.prototype.createUser = function (options) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if(!util.auth(options)) {
      ...
      errStr = "createUser: you have provided password.";
      log(errStr);
      reject(errStr)
    }  else {
      ...
      self.existsUser(usernameOptions)
        .then(function(results) {
          if(results.isExists) {
            reject({success: false, message: "exists user " + options.username});
          } else {
            ...
            self.collectionDb.insertOne(insertOptions)
              ...
              .catch(function(err) {
                logError("createUser: create user error.");
                reject(err);
              })
          }
        })
        .catch(function(err) {
          logError("createUser: find user exists error.");
          reject(err);
        })
    }
  })
